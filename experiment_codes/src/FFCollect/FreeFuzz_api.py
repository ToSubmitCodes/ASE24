...
from Futils.converter import str_to_bool
import os
...
if __name__ == "__main__":
    ...
    if library.lower() in ["pytorch", "torch"]:
        ...
        from Futils.skip import need_skip_torch
        ...
        from Futils.skip import need_skip_tf
        import json
        ...
        if need_skip_tf(api_name): pass
            ...
            for _ in range(each_api_run_times):
                ...
                api.mutate(enable_value, enable_type, enable_db)
                if crash_oracle:
                    err = MyTF.test_with_oracle(api, OracleType.CRASH)
                    if err is None:
                        res.append('pass')
                    else:
                        res.append('fail')
        ...
        json.dump(res,open(f'freefuzzResult/{api_name}_jax','w'))
    else:
