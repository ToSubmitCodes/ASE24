...
maskDisk = json.load(open('../MaskDict.json'))
...
def getCons(clsName,argDict):
    consMask = maskDisk[clsName]['Cons']
    return getPartialDict(clsName,argDict,consMask)
    
def getDisc(clsName,argDict):
    discMask = maskDisk[clsName]['Disc']
    return getPartialDict(clsName,argDict,discMask)
...
def vloumn(clsName):
    discMask = maskDisk[clsName]['Disc']
    v = 1
    for index in discMask:
        v *= len(paramInfo[clsName][index]['valueSpace'])
    return v

def getDiscValueList(clsName):
    discMask = maskDisk[clsName]['Disc']
    return getPartial(clsName,discMask)